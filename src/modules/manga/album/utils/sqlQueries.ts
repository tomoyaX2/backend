...
export const getDataWithRelations = (dataToSelect, orderBy) => {
  ...
  return (
    ...
    `"Album__rates"."id" AS "Album__rates_id", "Album__rates"."rate" AS "Album__rates_rate", "Album__rates"."albumId" AS "Album__rates_albumId"` +
    `FROM "album" "Album" ` +
    ...
    `LEFT JOIN "rate" "Album__rates" ON "Album__rates"."albumId"="Album"."id"` +
    `LEFT JOIN "album_tags" "Album_Album__tags" ON "Album_Album__tags"."album_id"="Album"."id" ` +
    `LEFT JOIN "tag" "Album__tags" ON "Album__tags"."id"="Album_Album__tags"."tag_id"` +
    `WHERE "Album"."id" IN (${queryIdSymbols}) ORDER BY "Album"."${orderBy}" DESC`
  );
};
